...
import { ArrowDownIcon, ArrowUpIcon, Download, RefreshCcw, Search, CheckCircle } from 'lucide-react';
...
export default function CashFlowPage() {
  ...
  const fetchProjects = async () => {
    try {
      ...
      const res = await fetch(`${API_URL}/projects-data/projects`, {
  ...
  const fetchReport = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      let url = `${API_URL}/reports/cashflow?start_date=${startDate}&end_date=${endDate}`;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-gray-500 mt-1 text-sm">
            Laporan Arus Kas (Pemasukan & Pengeluaran Lunas)
          </p>
        </div>
        <div className="flex gap-2">
          <button onClick={fetchReport} disabled={loading} className="flex items-center gap-2 border border-gray-300 text-gray-700 px-4 py-2 rounded-lg text-sm font-medium hover:bg-gray-50 transition-colors">
            <RefreshCcw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} />
            Refresh
          </button>
          <button onClick={exportToExcel} disabled={!report || loading} className="flex items-center gap-2 bg-emerald-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-emerald-700 transition-colors disabled:opacity-50">
      ...
      <div className="bg-white rounded-xl shadow-sm p-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
          ...
          <button
            onClick={fetchReport}
            disabled={loading}
            className="flex items-center justify-center gap-2 bg-accent hover:bg-accent-hover text-white px-4 py-2 rounded-lg text-sm transition-colors"
          >
      ...
      {report && (
        <>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-white rounded-xl shadow-sm p-5 flex flex-col justify-between border-t-4 border-emerald-500">
            ...
            <div className="bg-white rounded-xl shadow-sm p-5 flex flex-col justify-between border-t-4 border-rose-500">
              <div className="flex justify-between items-center mb-2">
              ...
              <div className="text-2xl font-bold text-gray-800">
                {formatIDR(report.total_expense)}
              </div>
            </div>

            <div className={`bg-white rounded-xl shadow-sm p-5 flex flex-col justify-between border-t-4 ${report.net_balance >= 0 ? 'border-blue-500' : 'border-orange-500'}`}>
              <div className="flex justify-between items-center mb-2">
