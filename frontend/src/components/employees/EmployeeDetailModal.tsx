        ...
        <div className="p-6 space-y-5">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-3">
              ...
              <div>
                ...
                <p className="text-gray-900">{(employee as any).email || '-'}</p>
              </div>
              <div>
                <h3 className="text-sm font-semibold text-gray-600">Telepon</h3>
                <p className="text-gray-900">{(employee as any).phone || '-'}</p>
              </div>
              <div>
                <h3 className="text-sm font-semibold text-gray-600">NIK</h3>
                <p className="text-gray-900">{(employee as any).nik || '-'}</p>
              </div>
              <div>
                <h3 className="text-sm font-semibold text-gray-600">Alamat</h3>
                <p className="text-gray-900">{(employee as any).address || '-'}</p>
              </div>
            </div>
            <div className="space-y-3">
              <div>
              ...
              <div>
                ...
                <p className="text-gray-900">{(employee as any).employment_type || '-'}</p>
              </div>
              <div>
                <h3 className="text-sm font-semibold text-gray-600">Tanggal Masuk</h3>
                <p className="text-gray-900">{(employee as any).join_date || '-'}</p>
              </div>
              <div>
                <h3 className="text-sm font-semibold text-gray-600">Jenis Kelamin</h3>
                <p className="text-gray-900">{(employee as any).gender || '-'}</p>
              </div>
              <div>
                <h3 className="text-sm font-semibold text-gray-600">Status Pernikahan</h3>
                <p className="text-gray-900">{(employee as any).marital_status || '-'}</p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-3">
              ...
              <p className="text-gray-900">{(employee as any).emergency_contact_name || '-'} ({(employee as any).emergency_contact_relation || '-'})</p>
              <p className="text-gray-900">{(employee as any).emergency_contact_phone || '-'}</p>
            </div>
            {canAccessFinancial && (
              <div className="space-y-3">
                <h3 className="text-sm font-semibold text-gray-600">Data Finansial</h3>
                <p className="text-gray-900">Gaji/Hari: {employee.daily_salary ? `Rp ${parseFloat(employee.daily_salary.toString()).toLocaleString('id-ID')}` : '-'}</p>
                <p className="text-gray-900">Saldo Pinjaman: {employee.loan_balance ? `Rp ${parseFloat(employee.loan_balance.toString()).toLocaleString('id-ID')}` : '-'}</p>
                <p className="text-gray-900">Bank: {(employee as any).bank_name || '-'}</p>
                <p className="text-gray-900">No. Rekening: {(employee as any).bank_account_number || '-'}</p>
              </div>
            )}
          </div>
        </div>
